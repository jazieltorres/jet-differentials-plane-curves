--------------------------------------------------------------------------------
-- Example 8: computational verification
--
-- Family:
--
--   f = x_0^(2n) + x_0^n*x_1^n + x_1^(2n) + x_1*x_2^(2n-1),
--   n >= 7.
--
-- IMPORTANT:
...
--       factorized parallel locus stated in Example 8.
--
...
print "============================================================";
print("EXAMPLE 8 VERIFICATION WITH n = " | toString n 
    | ", d = " | toString d);
print "============================================================";
...
P = QQ[x_0..x_2];
f = x_0^(2*n) + x_0^n*x_1^n + x_1^(2*n) + x_1*x_2^(2*n-1);
...
if n == 7 then (
    ...
    assert(#select(lowGenDegrees, dd -> dd == {9,1}) == 2);
) else (
    assert(numColumns Glow == 3);
    assert(#select(lowGenDegrees, dd -> dd == {n+2,1}) == 2);
    assert(#select(lowGenDegrees, dd -> dd == {n+2,2}) == 1);
);
...
if n == 7 then (
    expectedComponents = {
        ...
        ideal (x_2*a_1+13*x_1*a_2)
        }
) else (
    expectedComponents = {
        ideal (x_2),
        ideal (x_1*a_0-x_0*a_1),
        ideal (x_2*a_1+(2*n-1)*x_1*a_2)
        }
);
...
print " ";
print "============================================================";
print("ALL COMPUTATIONAL CHECKS FOR EXAMPLE 8 PASSED FOR n = " |
      toString n | ".");
print "============================================================";
