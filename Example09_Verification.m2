--------------------------------------------------------------------------------
-- Example 9: computational verification
--
-- Family:
--
--   f = x_0^(2*n+1)  + x_0^n*x_1^(n+1) + x_1^(2*n+1) + x_1*x_2^(2*n),
--   n >= 7.
--
-- IMPORTANT:
...
--
-- For a chosen n, this file checks:
--
...
--           [sigma_1, sigma_2, e]              if n = 7,
--       or
--           [sigma_1, sigma_2, sigma_3, e]     if n >= 8,
--       agrees set-theoretically, after multigraded saturation, with the
--       factorized parallel locus stated in Example 9.
--
...
print "============================================================";
print("EXAMPLE 9 VERIFICATION WITH n = " | toString n 
    | ", d = " | toString d);
print "============================================================";
...
if n == 7 then (
    ...
    assert(#select(lowGenDegrees, dd -> dd == {9,1}) == 1);
    assert(#select(lowGenDegrees, dd -> dd == {10,1}) == 1);
) else (
    assert(numColumns Glow == 3);
    assert(#select(lowGenDegrees, dd -> dd == {n+2,1}) == 1);
    assert(#select(lowGenDegrees, dd -> dd == {n+3,1}) == 1);
    assert(#select(lowGenDegrees, dd -> dd == {n+3,2}) == 1);
);
...
if n == 7 then (
    expectedComponents = {
        ...
        ideal (x_2*a_1 + 14*x_1*a_2)
        }
) else (
    expectedComponents = {
        ideal (x_2),
        ideal (x_1*a_0 - x_0*a_1),
        ideal (x_2*a_1 + 2*n*x_1*a_2)
        }
);
...
print " ";
print "============================================================";
print("ALL COMPUTATIONAL CHECKS FOR EXAMPLE 9 PASSED FOR n = " |
      toString n | ".");
print "============================================================";
